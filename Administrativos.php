	...
	switch($accion){
			case "btnRegistrar":
				if(!empty($_POST["ID_Data"]) and !empty($_POST["Nombre"]) and !empty($_POST["Apellido_pat"]) and !empty($_POST["Apellido_mat"]) and !empty($_POST["Carrera"]) and !empty($_POST["Num_control"]) and !empty($_POST["Telefono"])){
					$query = "INSERT INTO Administrativo (ID_Data, Nombre, Apellido_pat, Apellido_mat, Carrera, Num_control, Telefono) VALUES ('$ID_Data', '$Nombre', '$Apellido_pat', '$Apellido_mat', '$Carrera', '$Num_control', '$Telefono')";
					$database->exec($query);
			...
			case "btnModificar":
				if(!empty($_POST["ID_Data"]) and !empty($_POST["Nombre"]) and !empty($_POST["Apellido_pat"]) and !empty($_POST["Apellido_mat"]) and !empty($_POST["Carrera"]) and !empty($_POST["Num_control"]) and !empty($_POST["Telefono"])){
					$query = "UPDATE Administrativo SET Nombre='$Nombre', Apellido_pat='$Apellido_pat', Apellido_mat='$Apellido_mat', Carrera='$Carrera', Num_control='$Num_control', Telefono='$Telefono' WHERE ID_Data='$ID_Data'";
					$database->exec($query);
			...
			break;
			case "btnEliminar":
				$query = "DELETE FROM Administrativo WHERE ID_Data='$ID_Data'";
...
<body>
	...
	<div class="container">
		<form action="" method="post" ectype="multipart/form-data" >
			...
			<button value="btnCambiar" type="submit" name="accion">Al módulo carreras</button>
		</form>
		<table class="table">
  			<thead>
    				<tr>
  			...
  			<tbody>
				<?php
					...
					$result = $db->query('SELECT * FROM Administrativo');
					while ($row = $result->fetchArray(SQLITE3_ASSOC)) {?>
						<tr>
						<th scope="row"><?= $row['ID_Administrativo'] ?></th>
						<td><?= $row['ID_Data'] ?></td>
						<td><?= $row['Nombre'] ?></td>
